...
parser.add_argument(
    "--save_path", default="data/", type=str, help="path of saved log files"
)
parser.add_argument(
    "--seed", type=int, default=0, help="random seed for environment initialization"
)
parser.add_argument(
    "--tom",
    action="store_true",
...
parser.add_argument(
    "--model", type=str, default="gpt-4-turbo-preview", help="base LM to use"
)
parser.add_argument(
...
parser.add_argument(
    "--tool_per_agent", type=int, default=2, help="kinds of tool each agent has"
)
parser.add_argument(
    "--temperature", type=float, default=0, help="temperature used by base LM"
)
parser.add_argument("--max_step", type=int, default=30, help="maximum steps allowed")
parser.add_argument(
    "--exp_name", type=str, default="default_exp", help="exp name to save"
)
# model
...
DATA_PATH = (
    os.path.join(args.save_path, args.model, args.exp_name, "seed" + str(args.seed))
    + "/"
)
...
env = DragonTextEnv(
    seed=seed,
    include_agent_action=args.include_agent_action,
    allow_comm=allow_comm,
    act_and_comm=act_and_comm,
    tool_per_agent=args.tool_per_agent,
)
...
initial_actions = {
    "alpha": Action.go_to(int(initial_node)),
    "bravo": Action.go_to(int(initial_node)),
    ...
    for agent_id in chat_agents.keys():
        ...
        if round == 1 and not belief:
            _, reward, done, info, obs_text = env.step(
                agent_id, 0, initial_actions, communications
            )
        ...
        initial_actions[agent_id], communications[agent_id] = env.decode_action(
            chat_output[agent_id]
        )
            ...
            if initial_actions[agent_id].node() is not None:
                ...
                ToM1st = chat_agent.ask(
                    ...
                    + "Do you know the current contents of room {room_id}?".format(
                        player_id=target_id, room_id=initial_actions[agent_id].node().id
                    )
                )
                # second-order ToM
        ...
        record = {
            ...
            "action": initial_actions[agent_id].name.replace("_", " "),
