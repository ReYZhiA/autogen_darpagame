import pandas as pd
from initial_paper.gymdragon.gym_dragon.envs import MiniDragonEnv
from initial_paper.gymdragon.gym_dragon.core import Region, Agent, Tool, Bomb
from initial_paper.gymdragon.gym_dragon.wrappers import MiniObs
import openai
import time
import json
import os
...
class DragonTextEnv:
    ...
    def step(self, agent_id, round, initial_actions, communications):
        ...
        prev_agent_health = {
            agent.id: agent.health for agent in self.env.agents.values()
        }
        ...
        if action is None:
            ...
            if agent.bomb:
                ...
                sequence = "-".join(
                    [
                        COLOR_TO_STR[x]
                        for x in agent.bomb._full_sequence[agent.bomb._current_step :]
                    ]
                )
            ...
            if (
                self.env._get_action_mask(agent_id)[action]
                or action.node().id == agent.node.id
            ):
                room_id = str(action.node().id)
                room_agents = " and ".join([str(x.id) for x in action.node().agents])
                room_bombs = " and ".join(
                    ["Bomb " + str(x.id) for x in action.node().bombs]
                )
                ...
                if agent.bomb:
                    if tool.color == agent.bomb.color:
                        ...
                        sequence = "-".join(
                            [
                                COLOR_TO_STR[x]
                                for x in agent.bomb._full_sequence[
                                    agent.bomb._current_step :
                                ]
                            ]
                        )
        ...
        if self.include_agent_action:
            ...
            for a in initial_actions.keys():
                if initial_actions[a].name == "remove_bomb_beacon":
                    text += 'Player {id}: "{action}". '.format(
                        id=a, action="Sent a communication message to the Team."
                    )
                elif initial_actions[a].name == "remove_help_beacon":
                    text += 'Player {id}: "{action}". '.format(
                        id=a, action="Invalid action."
                    )
                else:
                    text += 'Player {id}: "{action}". '.format(
                        id=a, action=initial_actions[a].name.replace("_", " ")
                    )
    ...
    def decode_action(self, chat_output):
        ...
        if self.act_and_comm:
            if (
                len(chat_output.split("Message to Team:")) > 1
                and len(chat_output.split('"')) > 1
            ):
                comm = chat_output.split("Message to Team:")[1].split('"')[1]
                chat_output = (
                    chat_output.split("Message to Team:")[0]
                    + chat_output.split("Message to Team:")[1].split('"')[2]
                )
            else:
                comm = ""
    ...
    def load(self, saved_files, ending_round=999):
        ...
        chat_agents = {
            "alpha": ChatAgent(agent_id="alpha"),
            "bravo": ChatAgent(agent_id="bravo"),
            "charlie": ChatAgent(agent_id="charlie"),
        }
        initial_actions = {
            "alpha": Action.go_to(0),
            "bravo": Action.go_to(0),
            "charlie": Action.go_to(0),
        }
        ...
        for r in json_data:
            if "action" in r.keys():
                ...
                _, reward, done, info, obs_text = self.step(
                    agent_id, round, initial_actions, communications
                )
                round = r["round"]
            else:
                agent_id = r["agent_id"]
                initial_actions[agent_id], communications[agent_id] = (
                    self.decode_action(r["chat_output"])
                )
                _, reward, done, info, obs_text = self.step(
                    agent_id, round, initial_actions, communications
                )
        ...
        for agent_id in ["alpha", "bravo", "charlie"]:
            ...
            initial_actions[agent_id], communications[agent_id] = self.decode_action(
                file["message_history"][-2]["content"]
            )
    ...
    def to_csv(self, saved_file, output_path):
        ...
        for r in json_data:
            ...
            _, reward, done, info, obs_text = self.step(
                agent_id, initial_actions, communications
            )
            round = r["round"]
            row = [
                round,
                agent_id,
                initial_actions[agent_id],
...
class ChatAgent:
    def __init__(
        ...
        if self.allow_comm:
            ...
            self.last_belief = INITIAL_BELIEF.format(
                agent_id=agent_id, initial_bomb=initial_bomb, initial_node=initial_node
            )
        else:
            BACKGROUND_PROMPT = BACKGROUND_PROMPT_NOCOMM
            self.last_belief = INITIAL_BELIEF_NOCOMM.format(
                agent_id=agent_id, initial_bomb=initial_bomb, initial_node=initial_node
            )
        if message_history is None:
            self.message_history = [
                {
                    "role": "system",
                    "content": "You are playing a text game with the user.",
                },
                {
                    "role": "user",
                    "content": BACKGROUND_PROMPT.format(agent_id=agent_id),
                },
                # {"role": "user", "content": INSTRUCT_PROMPT},
            ]
            if self.belief:
                self.message_history.append(
                    {"role": "user", "content": self.last_belief + INITIAL_PROMPT}
                )
        else:
    ...
    def update_history(self, text):
        ...
        if self.belief:
            ...
            if self.model == "gpt-3.5-turbo":
                ...
                self.message_history.append(
                    {"role": "user", "content": last_action + text}
                )
                new_belief = self.makeAPIcall()
                self.message_history.pop()
                self.message_history.append(
                    {"role": "assistant", "content": last_action}
                )
                self.message_history.append(
                    {
                        "role": "user",
                        "content": "Below is your current belief about game state based on your previous observations about the environment and interactions with your teammates. "
                        + new_belief
                        + INITIAL_PROMPT,
                    }
                )
                ...
                self.message_history.pop()
                self.message_history.append(
                    {"role": "user", "content": new_belief + INITIAL_PROMPT}
                )
    ...
    def save(self, data_path):
        ...
        path = (
            data_path
            + "{model}_{temperature}_{allow_comm}_{agent_id}_{timestr}.json".format(
                timestr=timestr,
                agent_id=self.agent_id,
                allow_comm=self.allow_comm,
                model=self.model,
                temperature=self.temperature,
            )
        )
